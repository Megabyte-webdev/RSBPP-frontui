...
const GradeDetails = ({data, setScore, score, setRemark, remark}) => {
  ...
  useEffect(() => {
    ...
    //   setDescription(data?.text_submission || "");
    }
  }, [data, getAllFaculty]);
...
{data?.file_submission ? data?.file_submission?.split("/")[1].slice(0, 8)+"...": 'No files Submitted'}</p>
      ...
      <input type="text" placeholder="Enter a score" onInput={(e)=>handleScore(e)} value={score} className='mx-2 bg-gray-200 placeholder:text-gray-500 py-2 px-3 w-full text-sm'/>
      </div>
